...
import static com.zhongmei.yunfu.db.entity.trade.PaymentItem.$.payModeId;
...
public class GroupOrderCenterListManager extends OrderCenterListManager {
    private static final String TAG = GroupOrderCenterListManager.class.getSimpleName();

    @Override
    public Where<Trade, String> generateTradeBusinessTypeWhere(QueryBuilder<Trade, String> queryBuilder) throws Exception {
        return queryBuilder.where().eq(Trade.$.businessType, BusinessType.GROUP);
    }
    ...
    private List<TradeCustomer> getTradeCustomerList(DatabaseHelper helper, String tradeUuid) {
        ...
        try {
            List<TradeEarnestMoney> tradeEarnestMonies = helper.getDao(TradeEarnestMoney.class)
                    .queryBuilder()
                    .where().eq(TradeEarnestMoney.$.tradeId, tradeId).and().eq(TradeEarnestMoney.$.statusFlag, StatusFlag.VALID).query();
            return tradeEarnestMonies;
        } catch (Exception e) {
            Log.e(TAG, e.toString());
        }
        return Collections.emptyList();
    ...
    private List<PaymentVo> listPaymentVo(DatabaseHelper helper, String tradeUuid) throws Exception {
        ...
        for (Payment payment : paymentList) {
            ...
            List<PaymentItem> listPaymentItems = itemDao.queryBuilder()
                    .selectColumns(paySource, payStatus, faceAmount, changeAmount, payModeId)
