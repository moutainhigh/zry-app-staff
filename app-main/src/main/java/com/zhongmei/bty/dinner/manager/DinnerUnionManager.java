...
public class DinnerUnionManager {
    ...
    private static UnionTradeItemOperationReq createUnionOperateionReq(TradeVo tradeVo, DinnertableTradeInfo mainTradeInfo, List<DishDataItem> selectedItems) {
        ...
        if (tradeVo.isUnionMainTrade()) {             UnionTradeItemOperationReq.BatchTradeItemOperationRequest batchTradeItemOperationRequest = new UnionTradeItemOperationReq.BatchTradeItemOperationRequest();
        ...
        } else if (tradeVo.isUnionSubTrade()) {                                                                                                                                                             boolean hasBatch = DinnerDishManager.getInstance().hasBatchDish(selectedItems);
            ...
            if (selectedItemIds.isEmpty() || selectedItemIds.size() < selectedItems.size() || !kitchenUUIDs.isEmpty()
                    || hasBatch) {                 TradeUnionModifySubWarpReq tradeUnionSubReq = createUnionSubModifyReq(tradeVo, mainTradeInfo, newSelectedUUids);
