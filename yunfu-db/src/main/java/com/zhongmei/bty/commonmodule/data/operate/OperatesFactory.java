    ...
    public static <T extends IOperates> T create(Class<T> classType) {
        ...
        if (obj == null) {
            ...
            try {
                ...
                Log.w("OperatesFactory", String.format("dynamic proxy error: %s", obj.getClass().getName()));
            }
        }
    ...
    static class DynamicProxy implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            if (obj.getClass().getSimpleName().endsWith("DalImpl")) {
                if (isMainThread()) {
                    Log.e("OperatesFactory", logMsg);
                } else {
                    if (diff >= 500) {
                        Log.w("OperatesFactory", logMsg);
                    } else {
                        Log.i("OperatesFactory", logMsg);
                    }
                }
            }
            return result;
        ...
        private String getFromMethod() {
            ...
            for (StackTraceElement a : temp) {
                if (a.getClassName().contains("com.zhongmei.bty") && !a.getClassName().contains(OperatesFactory.class.getName())) {
                    trace = a;
                    break;
                }
            }
