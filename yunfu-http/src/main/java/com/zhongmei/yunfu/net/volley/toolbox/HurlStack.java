...
//import com.zhongmei.bty.dnscache.DNSCache;
...
public class HurlStack implements HttpStack {
    ...
    public HttpResponse performRequest(Request<?> request,
        ...
        String originalHost = parsedUrl.getHost();
        // 同步接口获取IP
		/*String ip = DNSCache.getInstance().getIpByHost(originalHost);
		if (!TextUtils.isEmpty(ip)) {
			parsedUrl = new URL(originalUrl.replaceFirst(originalHost, ip));
		}
		Log.i("DNSCache HurlStack",originalUrl+"-->"+parsedUrl.toString());*/
        HttpURLConnection connection = openConnection(parsedUrl, request);
		/*if (!TextUtils.isEmpty(originalHost)) {
			connection.setRequestProperty("Host", originalHost);
		}*/

        //add 2016.01.20 for verifyPay request too quickly
        if (request.getTag() != null && "verifyPay".equals((String) request.getTag())) {
            ...
            case Method.DEPRECATED_GET_OR_POST:
                ...
                if (postBody != null) {
                    ...
                    DataOutputStream out = new DataOutputStream(
                            getOutputStream(connection, request, postBody));
                    out.write(postBody);
                    out.close();
                }
                break;
    ...
    private static void addBodyIfExists(HttpURLConnection connection,
        ...
        if (body != null) {
            ...
            DataOutputStream out = new DataOutputStream(
                    getOutputStream(connection, request, body));
            out.write(body);
            out.close();
        }
    }

    private static OutputStream getOutputStream(final HttpURLConnection connection, final Request<?> request, final byte[] body) throws IOException {
        if (HttpConstant.isAllowCompress(connection.getURL().toString(), body)) {
            connection.setRequestProperty(HttpConstant.KRY_COMPRESS_TYPE, "gzip");
            DataOutputStream out = new DataOutputStream(connection.getOutputStream()) {
                @Override
                public void close() throws IOException {
                    super.close();
                    Log.i("HurlStack", String.format("[%s]gzip: %d/%d(%.1f%%)",
                            getHeadersNoError(request).get(HttpConstant.YF_API_MSG_ID),
                            size(),
                            body.length,
